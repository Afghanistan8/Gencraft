...
const QUESTIONS = [
  ...
  /* ─── BASICS ─── */
  { cat:'BASICS', q:'What is GenLayer?', opts:['A centralised AI company','AI-native blockchain with','A Layer 2 scaling for Bitcoin','A DeFi lending protocol'], ans:1, exp:'GenLayer is an AI-native blockchain introducing Intelligent Contracts — AI-powered smart contracts connected to the internet.' },
  ...
  { cat:'BASICS', q:'What does GenLayer deliver beyond Bitcoin and Ethereum?', opts:['Much faster transactions','Trustless decision-making','Much lower transaction fees','Better on-chain privacy'], ans:1, exp:'Bitcoin = trustless money. Ethereum = trustless computation. GenLayer = trustless decision-making.' },
  ...
  { cat:'BASICS', q:'What problem do traditional smart contracts have with real-world data?', opts:['Too slow to ever confirm','Cannot natively access the internet','Far too expensive to run','Cannot handle real-world tokens'], ans:1, exp:'Traditional blockchains are isolated — they need external oracles to access real-world data.' },
  ...
  { cat:'BASICS', q:'How does GenLayer describe its core role?', opts:['Fastest chain for AI','A synthetic jurisdiction and','Cheapest AI platform','A proof-of-work miner'], ans:1, exp:'GenLayer is a synthetic jurisdiction — a decentralised digital court that resolves disputes via AI validators.' },
  ...
  { cat:'BASICS', q:'What does GenLayer say most new blockchain dApps are?', opts:['Truly revolutionary breakthroughs','Mostly forks or incremental improvements','Completely original inventions','Government approved protocols'], ans:1, exp:'GenLayer observes that most new dApps are forks because traditional smart contracts cannot handle real-world complexity.' },
  ...
  { cat:'BASICS', q:'How does GenLayer position itself relative to Layer 2 blockchains?', opts:['It is a Layer 2','It is a Layer 1 with','It is a sidechain','It is an off-chain solution'], ans:1, exp:'GenLayer is a Layer 1 blockchain with AI-native capabilities built directly into its core.' },
  { cat:'BASICS', q:'What does GenLayer replace in the traditional blockchain stack?', opts:['The entire wallet layer','Traditional deterministic contracts','The ERC token standard','The proof-of-work mining algorithm'], ans:1, exp:'GenLayer replaces deterministic smart contracts with Intelligent Contracts that can reason, interpret and fetch real-world data.' },
  ...
  { cat:'CONSENSUS', q:'What happens when validators disagree?', opts:['Transaction is rejected','An appeal opens and validator','Leader automatically wins','Returns to original sender'], ans:1, exp:'Disagreement opens an appeal window — participants post a bond to double the validator set.' },
  ...
  { cat:'CONSENSUS', q:'What staking model does GenLayer use?', opts:['Nominated Proof of Stake','Liquid Proof of Stake','Delegated Proof of Stake','Pure Proof of Stake'], ans:2, exp:'GenLayer uses delegated Proof-of-Stake where validators are randomly selected subsets.' },
  { cat:'CONSENSUS', q:'Why is Optimistic Democracy needed?', opts:['Traditional consensus is too slow','Blockchains cannot handle','Requires less network energy','More decentralised by design'], ans:1, exp:'AI models produce varied outputs for the same input. Optimistic Democracy handles this through validator voting.' },
  { cat:'CONSENSUS', q:'What is the Equivalence Principle?', opts:['Outputs must be word identical','Outputs just need to be equivalent enough','All token values must match','Contracts must match Solidity'], ans:1, exp:'The Equivalence Principle allows for AI\'s natural variation — outputs just need to be close enough.' },
  { cat:'CONSENSUS', q:'What does the leader validator do in a transaction round?', opts:['Earns double the rewards','Proposes the output for','Is permanently the leader','Pays a transaction fee'], ans:1, exp:'The leader proposes the output. The other four validators independently compare against their own results and vote.' },
  ...
  { cat:'CONSENSUS', q:'What does Optimistic Democracy bridge?', opts:['Speed vs security','Determinism and real-world','Privacy vs transparency','Central vs decentralised'], ans:1, exp:'Optimistic Democracy bridges the gap between blockchain determinism and the complexity of intelligent real-time decision-making.' },
  { cat:'CONSENSUS', q:'How are validators selected in Optimistic Democracy?', opts:['By stake size only','By reputation score','Randomly from staking pool','By community vote'], ans:2, exp:'Validators are randomly selected subsets from the delegated Proof-of-Stake pool — preventing centralisation.' },
  { cat:'CONSENSUS', q:'What does a validator do if they disagree with the leader\'s output?', opts:['Accept it regardless','Vote to reject it','Leave the network','Post a complaint on-chain'], ans:1, exp:'Validators independently evaluate the output and vote to reject if it does not match their own result — triggering re-evaluation.' },

  /* ─── INTELLIGENT CONTRACTS ─── */
  { cat:'INTELLIGENT CONTRACTS', q:'What can Intelligent Contracts do that traditional ones cannot?', opts:['Execute transfers faster','Fetch live internet data directly','Process cheaper transactions','Store more data'], ans:1, exp:'Intelligent Contracts fetch live web data directly on-chain — no external oracles required.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What type of language can Intelligent Contracts interpret?', opts:['Only Solidity-style code','Only strict formal code','Natural language instructions','Only binary encoded data'], ans:2, exp:'Intelligent Contracts interpret natural language — contracts can be written in plain English.' },
  { cat:'INTELLIGENT CONTRACTS', q:'How fast can GenLayer resolve prediction markets?', opts:['Takes about 24 hours','Under 1 hour','Takes about 1 week','Resolves instantly'], ans:1, exp:'GenLayer resolves markets in under 1 hour at less than $1 per market.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What is Greyboxing in GenLayer?', opts:['A data privacy protection feature','Defence against adversarial AI attacks','A scheduled token burn mechanism','A cross-chain asset bridge protocol'], ans:1, exp:'Greyboxing mitigates adversarial attacks on AI models within the multi-validator system.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What does GenLayer\'s Intelligent Oracle do?', opts:['Provides randomness','Reads and reasons with live','Connects to Chainlink','Manages bridges'], ans:1, exp:'Intelligent Oracles let dApps read and reason with live web data trustlessly.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What replaces external oracles in GenLayer?', opts:['External Chainlink price feeds','Centralised third-party APIs only','Built-in web access in contracts','Manual human data reporters'], ans:2, exp:'Intelligent Contracts eliminate the need for external oracles by fetching and verifying live data directly on-chain.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What types of data can Intelligent Contracts process?', opts:['Only numerical on-chain data','Only stored on-chain data','Text, images and qualitative evidence','Only tokenised digital assets'], ans:2, exp:'Intelligent Contracts process text, images, qualitative evidence, and unstructured data — far beyond traditional contracts.' },
  { cat:'INTELLIGENT CONTRACTS', q:'How much does GenLayer charge per market resolution?', opts:['Around $10 per call','Around $5 per call','Under $1 per resolution','Only $0.001 in theory'], ans:2, exp:'Market resolutions cost under $1 on GenLayer — making them economically viable at scale.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What type of operations can Intelligent Contracts handle that traditional ones cannot?', opts:['Only fully deterministic operations','Only standard financial operations','Non-deterministic real-world operations','Only stored on-chain data reads'], ans:2, exp:'Intelligent Contracts handle non-deterministic operations — they respond to varied real-world inputs unlike traditional contracts.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What does multimodality mean for future Intelligent Contracts?', opts:['Process multiple currencies','Work with images, audio and video too','Run on multiple blockchains','Serve multiple users at once'], ans:1, exp:'GenLayer plans to integrate full multimodality — enabling validators to work with images, audio, and video alongside text.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What ambiguous clause can an Intelligent Contract interpret?', opts:['Only numeric conditions','Ambiguous legal terms like','Only token conditions','Only time-based conditions'], ans:1, exp:'AI consensus interprets ambiguous legal terms like force majeure, turning subjective language into deterministic execution.' },
  { cat:'INTELLIGENT CONTRACTS', q:'What can an Intelligent Contract monitor for DeFi security?', opts:['Only internal on-chain events','Live news feeds to detect attacks','Only on-chain price feed data','Only on-chain governance votes'], ans:1, exp:'An Intelligent Contract can monitor crypto news sites to detect when a protocol is under attack and trigger emergency shutdown.' },

  /* ─── USE CASES ─── */
  { cat:'USE CASES', q:'How can GenLayer automate insurance payouts?', opts:['Only automating the premiums','AI-triggered payouts from real-world data','Only tokenising the policies','Via external third-party APIs'], ans:1, exp:'Intelligent Contracts monitor web data like drought alerts and trigger payouts automatically.' },
  { cat:'USE CASES', q:'What commercial dispute role can GenLayer play?', opts:['Small claims only','Decentralised AI arbitration','Acts as a government body','International trade only'], ans:1, exp:'GenLayer provides decentralised AI arbitration removing cost, delay and bias.' },
  { cat:'USE CASES', q:'How does GenLayer handle terms like force majeure?', opts:['Rejects ambiguous terms entirely','AI consensus makes them enforceable','Forwards them to human lawyers','Flags them for manual legal review'], ans:1, exp:'AI consensus interprets ambiguous legal terms turning subjective language into enforceable outcomes.' },
  { cat:'USE CASES', q:'What makes GenLayer ideal for AI agent commerce?', opts:['Stable token prices','A neutral trustless environment','Full Ethereum compatibility','Very fast block confirmation'], ans:1, exp:'GenLayer is purpose-built for AI agent commerce — agents can transact with humans and other agents.' },
  { cat:'USE CASES', q:'What DAO governance does GenLayer unlock?', opts:['Standard on-chain token-voting only','AI-powered DAOs with a constitution','DAOs requiring human arbitrators','Proof-of-work powered DAOs'], ans:1, exp:'GenLayer enables AI-powered DAOs where proposals are automatically checked against a constitutional framework.' },
  { cat:'USE CASES', q:'What can an Intelligent Contract do for a freelancer dispute?', opts:['Nothing, disputes need humans','Evaluate work and release','Only transfer the payment','Only log the dispute'], ans:1, exp:'An Intelligent Contract can evaluate submitted work against natural language criteria and execute payment automatically.' },
  { cat:'USE CASES', q:'How could GenLayer detect a drought for insurance purposes?', opts:['By checking satellite imagery','By fetching live weather and news data','By querying government APIs','By asking validators manually'], ans:1, exp:'An Intelligent Contract monitors publicly available web data including weather services and news to verify real-world conditions.' },
  { cat:'USE CASES', q:'What kind of oracle can GenLayer build that existing oracles cannot?', opts:['Price feed oracles only','Oracles that answer almost any','Weather data oracles only','Random number generators'], ans:1, exp:'Unlike traditional oracles limited to predefined datasets, GenLayer can build oracles that answer almost any question.' },
  { cat:'USE CASES', q:'How can GenLayer be used for on-chain identity verification?', opts:['Only via government APIs','AI verification using web','Only via social media','Only via token ownership'], ans:1, exp:'Intelligent Contracts can verify identity using web data and qualitative evidence enabling robust on-chain verification.' },
  { cat:'USE CASES', q:'What is unique about GenLayer\'s prediction market resolution?', opts:['They use off-chain settlement','AI resolves outcomes from web data in','They require human reporters','They only work for crypto prices'], ans:1, exp:'GenLayer resolves prediction markets in under 1 hour at under $1 using AI to verify outcomes directly from the web.' },
  { cat:'USE CASES', q:'How can GenLayer help content creator royalty distribution?', opts:['NFT minting only','Auto-verify usage and pay','Social media integration only','Video streaming only'], ans:1, exp:'Intelligent Contracts can monitor web data to verify content usage and automatically distribute royalties without intermediaries.' },
  { cat:'USE CASES', q:'How can GenLayer improve supply chain management?', opts:['Only by tracking tokens','Fetching shipping data and verifying','Connecting to existing ERP systems','By manual confirmation only'], ans:1, exp:'Intelligent Contracts can fetch real-world logistics data and verify delivery conditions, automating complex supply chain agreements.' },
  ...
  { cat:'TECHNICAL', q:'What is GenLayer Studio?', opts:['A mobile crypto wallet app','Browser-based Intelligent Contract IDE','An on-chain transaction explorer','A USB hardware security wallet'], ans:1, exp:'GenLayer Studio is the web-based IDE for writing, testing and deploying Intelligent Contracts.' },
  ...
  { cat:'TECHNICAL', q:'What privacy tech does Atoma Network bring?', opts:['Zero-knowledge proof systems','Trusted Execution Environments (TEEs)','Homomorphic encryption schemes','Secure multi-party computation'], ans:1, exp:'Atoma uses TEEs to isolate data during AI processing providing verifiable end-to-end privacy.' },
  ...
  { cat:'TECHNICAL', q:'What makes GenLayer non-deterministic compared to traditional blockchains?', opts:['It uses random number generation','AI reasoning produces varied outputs per call','It uses proof-of-work consensus mining','It relies entirely on human validator votes'], ans:1, exp:'AI models can produce varied outputs for the same input — making GenLayer non-deterministic by nature.' },
  { cat:'TECHNICAL', q:'What is model routing in GenLayer?', opts:['Routing to the cheapest network','Directing execution to the most','Load balancing across servers','Selecting only by stake weight'], ans:1, exp:'Model routing directs each Intelligent Contract execution to the most suitable AI model for the task.' },
  ...
  { cat:'TECHNICAL', q:'What cross-chain service does GenLayer support?', opts:['Only a bridge to Bitcoin','LayerZero for inter-blockchain','Only Wormhole for Solana','Only Polkadot parachains'], ans:1, exp:'GenLayer supports cross-chain communication via LayerZero — meaning its data can be used by any connected blockchain.' },
  { cat:'TECHNICAL', q:'What does GenLayer\'s Virtual Machine execute?', opts:['Solidity bytecode only','Python-based Intelligent Contracts','EVM-compatible transactions only','WebAssembly modules'], ans:1, exp:'The GenLayer Virtual Machine executes Python-based Intelligent Contracts supporting AI reasoning and web access natively.' },
  { cat:'TECHNICAL', q:'What security mechanism does GenLayer use against AI manipulation?', opts:['Only traditional code audits','Greyboxing — multi-validator AI defence','Only public bug bounties','Only centralised monitoring'], ans:1, exp:'Greyboxing distributes validation across multiple AI models reducing vulnerability to adversarial manipulation of any single model.' },
  { cat:'TECHNICAL', q:'What does genlayer-js allow developers to build?', opts:['Only mobile applications','Frontend apps that call Intelligent Contracts','Writing smart contracts in JavaScript','Building Solidity compiler tools'], ans:1, exp:'genlayer-js lets frontend developers build web applications that interact directly with GenLayer Intelligent Contracts.' },

  /* ─── ECOSYSTEM ─── */
  { cat:'ECOSYSTEM', q:'What does the GenLayer–Atoma partnership enable?', opts:['Much faster block times','Privacy-first confidential','Significantly cheaper gas','More advanced NFT features'], ans:1, exp:'The partnership enables privacy-first Intelligent Contracts resolving disputes with full data privacy.' },
  ...
  { cat:'ECOSYSTEM', q:'What does the Gaia partnership bring to GenLayer?', opts:['Many more network validators','Decentralised AI inference at the edge','Much cheaper distributed data storage','Better developer tooling and UX'], ans:1, exp:'The GenLayer-Gaia integration enables decentralised AI inference with on-chain arbitration at the network edge.' },
  { cat:'ECOSYSTEM', q:'What does GenLayer say about the current crypto ecosystem?', opts:['Growing extremely fast still','Mostly forks and minor improvements','Maturing only very slowly','Totally overcrowded with clones'], ans:1, exp:'GenLayer observes blockchain stagnation — most dApps are forks because smart contracts cannot handle real-world complexity.' },
  ...
  { cat:'ECOSYSTEM', q:'What does GenLayer identify as improbable for AI agents to rely on?', opts:['Decentralised blockchain technology','Centralised third parties and the legal system','Peer-to-peer decentralised networks','Purely token-based economies'], ans:1, exp:'GenLayer argues it is improbable that AI agents will use the slow expensive human-built legal system or trust centralised data providers.' },
  ...
  { cat:'ECOSYSTEM', q:'What does GenLayer offer developers building on its platform?', opts:['Only developer documentation','Testnets, grants and ecosystem tools','Direct hardware cost subsidies','Government-backed grants'], ans:1, exp:'GenLayer provides developer support programs, testnets, education initiatives, and ecosystem funding for builders.' },
  { cat:'ECOSYSTEM', q:'What partner provides private AI inference for GenLayer validators?', opts:['OpenAI via API','Atoma Network via TEEs','Anthropic via Claude API','Google DeepMind directly'], ans:1, exp:'Atoma Network provides private AI inference using Trusted Execution Environments ensuring data privacy during contract execution.' },
  { cat:'ECOSYSTEM', q:'What kind of applications does GenLayer enable that were previously impossible?', opts:['Only faster token transfers','Apps that interpret real events and adapt','Only better UX for existing dApps','Only making transactions cheaper'], ans:1, exp:'GenLayer enables apps that interpret real-world events, evaluate qualitative evidence, and adapt — impossible on deterministic blockchains.' },
  { cat:'ECOSYSTEM', q:'What does GenLayer call itself relative to AI agents needs?', opts:['The fastest AI network overall','The perfect neutral ground for AI agents to','The cheapest AI execution environment','The most secure AI validation ledger'], ans:1, exp:'GenLayer is built from the ground up to be the perfect place for AI Agents to conduct business — transacting and contracting trustlessly.' },
  { cat:'ECOSYSTEM', q:'Which other investors participated in GenLayer\'s seed round?', opts:['Sequoia and a16z','Arrington Capital, Node','Tiger Global and Paradigm','Multicoin and Pantera'], ans:1, exp:'Arrington Capital, Node Capital, TykheBlock, ZK Ventures and others participated alongside lead investor North Island Ventures.' },
];
